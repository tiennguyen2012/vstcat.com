...
class NotORM_Result extends NotORM_Abstract implements Iterator, ArrayAccess, Countable {
	...
	protected function query($query, $parameters) {
		if ($this->notORM->debug) {
			if (!is_callable($this->notORM->debug)) {
                ...
                $handle = fopen(STDERR, 'a+');
				fwrite($handle, "$backtrace[file]:$backtrace[line]:$debug\n");
	...
	*/
	function insert($data) {
		...
		if (empty($data[$this->primary]) && ($id = $this->notORM->connection->lastInsertId($this->notORM->structure->getSequence($this->table)))) {
