...
class OrderController extends Coco_Controller_Action_Default {
	...
	public function addAction() {
		...
		if (strtoupper ( $itemType ) == 'NEW_AND_TEMPLATE' && $this->_getParam ( 'result', null ) != 'success') {
			$this->_redirect ( $this->view->rurl ( array (
					'controller' => 'website',
					'action' => 'create' 
			) ) );
		} else {
	...
	public function detailAction() {
		...
		if ($domain) {
			...
			$isExistWebsite = $modelWebsite->isExistWebsiteByDomain ( $domain );
			$this->view->isExistWebsite = $isExistWebsite;
		}else{
			throw new Zend_Exception('Domain is not exist in your order.');
		}
	...
	/**
	 * This is action to add template and domain
	 * It was used when you setup template for domain is existed
	 * 
	 * @author tien.nguyen
	 */
	public function setTemplateAndDomainAction() {
		$templateId = $this->_getParam ( 'template-id' );
		$domain = $this->_getParam ( 'domain' );
		
		// check template id and domain
		if (empty ( $templateId ) || empty ( $domain )) {
			throw new Zend_Exception ( 'Template Id and Domain are required.' );
		}
		
		// add template and domain for basket
		$vtsBasket = new Vts_Basket ();
		$resTemplate = $vtsBasket->setTemplate ( $templateId );
		$resDomain = $vtsBasket->setDomain ( $domain );
		
		// if add template and domain success. Please go to next step
		if ($resTemplate && $resDomain) {
			$this->_redirect ( $vtsBasket->getNextStep () );
		}
	}
